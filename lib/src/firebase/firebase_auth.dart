...
class FirAuth {
  ...
  Future<void> signUp(
    ...
    try {
      ...
      await _createUserRecords(userId, email, pass, name, phone, position,);
  ...
  Future<int> getUserPosition(String userId) async {
    try {
      ...
      if (snapshot.exists) {
        ...
        return userData['position'] ?? 0; // Nếu không có position thì mặc định là 0
      }
    } catch (error) {
      print("Lỗi khi lấy position: $error");
    }
