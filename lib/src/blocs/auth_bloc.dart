...
class AuthBloc {
  ...
  bool isValid(String name, String email, String pass, String phone) {
    bool isValid = true;
    void safeAddError(BehaviorSubject<String> controller, String error) {
      if (!controller.isClosed) controller.sink.addError(error);
    }

    void safeAddValue(BehaviorSubject<String> controller, String value) {
      if (!controller.isClosed) controller.sink.add(value);
    }
    // Validate name
  ...
  Future<void> signUp(
    ...
    try {
      ...
      // Mặc định position = 1 khi đăng ký tài khoản
      int position = 1;

      // Attempt signup
      await _firAuth.signUp(email, pass, name, phone, position, onComplete);
    } catch (e) {
